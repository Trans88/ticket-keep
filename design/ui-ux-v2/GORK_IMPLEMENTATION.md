...
Android 修正实现与验证说明见 `ANDROID_GLASS_NOTES.md`：已接入真实背景模糊，请保留页面采样源与底栏效果之间的连接，不要退回仅半透明底色。

首页和“我的”的底栏已在 `index.html` 改为半透明玻璃胶囊，详见 `DESIGN_SPEC.md` 的“底部导航 v2.2”。请在现有 `HomeBottomBar.kt` 等实现上增量调整，不覆盖其他改动。

正文滚动到玻璃后方，导航独立悬浮；列表底部留足实测导航高度和系统安全区，末条记录可完整露出。浅深色分别处理，保留中心添加主动作和 48 dp 点击区。模糊的是背后内容，不是按钮本身。Android 的实际背景模糊方案需先验证兼容性、滚动性能与库依赖；无法可靠支持时用规范中的高不透明度回退，不能只调低背景 alpha 就声称完成背景模糊。

核对 320/360 dp、大字号、浅深色、三键/手势导航、键盘、底部面板层级、无模糊回退。网页原型不是 Android 渲染实现，不以浏览器效果替代真机验收。本次补充不改变收费策略。
